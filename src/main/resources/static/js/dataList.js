...
(function () {
    ...
    function fmtKm(n) {
        return Intl.NumberFormat('es-CO').format(n) + ' km'
    }
    ...
    async function fetchPage(cfg, state) {
        ...
        if (state.estado)
            url.searchParams.set('estado', state.estado)
    ...
    function persist(cfg, state) {
        try {
            sessionStorage.setItem(`dl:${cfg.storageKey}`, JSON.stringify(state))
        } catch {
        }
    }
    ...
    `
        ...
        qs('#dl-q', root).addEventListener('input', (ev) => {
            state.q = ev.target.value.trim();
            state.page = 1;
            load()
        })
        qs('#dl-estado', root).addEventListener('change', (ev) => {
            state.estado = ev.target.value;
            state.page = 1;
            load()
        })
        qs('#dl-prev', root).addEventListener('click', () => {
            if (state.page > 1) {
                state.page--;
                load()
            }
        })
        qs('#dl-next', root).addEventListener('click', () => {
            state.page++;
            load()
        })
