...
window.initMap = function(){ window.__cs.mapReady = true; };
(function(){
...
(function () {
    const run = () => {
        ...
        if (typeof window.iniciarInterfaz === 'function') window.iniciarInterfaz();
    };
...
function iniciarInterfazCrearSolicitud() {
    ...
    if (!hasPage) return;
    try { app.spinner(true, 'Cargando interfaz...'); } catch {}
    ...
    const calcularRuta = () => {
        ...
        if (!directionsService) {
            ...
            const onOk = (data) => {
                if (elDist) elDist.textContent = data.distancia || '—';
                if (elDur) elDur.textContent = data.duracion || '—';
                if (elKm) {
            ...
            if (window.jQuery) {
                jQuery.getJSON(url).done(function(resp){ if (resp && resp.estado === true && resp.salida) onOk(resp.salida); }).fail(function(){ if (elDist) elDist.textContent = '—'; if (elDur) elDur.textContent = '—'; });
            } else {
                fetch(url).then(r => r.ok ? r.json() : Promise.reject(r)).then(resp => { if (resp && resp.estado === true && resp.salida) onOk(resp.salida); }).catch(() => { if (elDist) elDist.textContent = '—'; if (elDur) elDur.textContent = '—'; });
            }
            return;
        }
    ...
    function loadSummary(){
        ...
        $.getJSON('/shoppi/peticiones').done(function(resp){
            if (resp && resp.estado === true && resp.salida) { $('#cs-total').text(resp.salida.total||0); $('#cs-mirando').text(resp.salida.mirando||0); }
        });
    }
    function ensureDataTables(){
        return new Promise((resolve) => {
            if (window.jQuery && jQuery.fn && typeof jQuery.fn.DataTable === 'function') return resolve(true);
            const addScript = (src) => new Promise((ok)=>{ const s=document.createElement('script'); s.src=src; s.onload=()=>ok(true); s.onerror=()=>ok(false); document.body.appendChild(s); });
            const chain = async ()=>{
                if (!window.jQuery) { const s=document.createElement('script'); s.src='https://code.jquery.com/jquery-3.7.1.min.js'; s.onload=()=>{}; document.body.appendChild(s); }
    ...
    async function initList(){
        if (!$table.length) { try { app.spinner(false); } catch {} ; return; }
        ...
        if (ok) {
            ...
            if (wrappers.length > 1) { wrappers.forEach((w,i)=>{ if (i < wrappers.length-1) w.remove(); }); }
            dt = $table.DataTable({
                ajax: { url:'/shoppi/peticiones', dataSrc: function(json){ if (json && json.estado===true && json.salida && Array.isArray(json.salida.items)) return json.salida.items; return []; } },
                columns: [
                    ...
                    { data:'watchersCount', render:function(v){ v = Number(v||0); var dot = '<span class="d-inline-block rounded-circle me-1" style="width:10px;height:10px;background:#0d6efd"></span>'; var count = v>1 ? '<span class="badge bg-primary">'+v+'</span>' : ''; return dot + count; } },
                    { data:'estado', render:function(v){ const map={ 'sin_asignacion':'secondary','mirando_peticion':'warning','asignando':'info','asignada':'success' }; const cls = map[String(v||'').toLowerCase()]||'secondary'; return '<span class="badge bg-'+cls+'">'+(v||'sin_asignacion')+'</span>'; } },
                    { data:null, orderable:false, render:function(row){ return '<div class="btn-group btn-group-sm" role="group">'+
            ...
            dt.on('xhr', function(){ try { app.spinner(false); } catch {} loadSummary(); });
            dt.on('init.dt', function(){ try { app.spinner(false); } catch {} });
            loadSummary();
            ...
            if (txtEl) txtEl.addEventListener('input', function(){ dt.search(this.value || '').draw(); });
            if (estEl) {
                ...
                const applyEstado = () => { const v = getEstadoValue(); dt.column(12).search(v ? '^'+v+'$' : '', true, false).draw(); };
            ...
            if (watchEl) {
                if (!window.__csWatchFilterAdded) {
                    $.fn.dataTable.ext.search.push(function(settings, data, dataIndex, rowData){
                        const min = Number((watchEl.value||'0')||0); const wc = Number(rowData.watchersCount||0);
                        return wc >= min;
                    });
                    window.__csWatchFilterAdded = true;
                }
                watchEl.addEventListener('input', function(){ dt.draw(); });
            }
        } else {
            ...
            fetch('/shoppi/peticiones').then(r=>r.ok?r.json():Promise.reject(r)).then(resp=>{
                ...
                items.forEach(row=>{
                    ...
                    const cells = [row.id,row.origen,row.destino,row.fecha,row.hora,row.kilos,row.carga,row.tipoVehiculo,row.distancia,row.duracion,row.km,
                        (function(v){ v=Number(v||0); const dot='<span class="d-inline-block rounded-circle me-1" style="width:10px;height:10px;background:#0d6efd"></span>'; const count=v>1?('<span class="badge bg-primary">'+v+'</span>'):''; return dot+count; })(row.watchersCount),
                        (function(v){ const map={ 'sin_asignacion':'secondary','mirando_peticion':'warning','asignando':'info','asignada':'success' }; const cls=map[String(v||'').toLowerCase()]||'secondary'; return '<span class="badge bg-'+cls+'">'+(v||'sin_asignacion')+'</span>'; })(row.estado),
                ...
                loadSummary();
                try { app.spinner(false); } catch {}
            }).catch(()=>{});
        }
    }
    initList();
    setTimeout(function(){ try { app.spinner(false); } catch {} }, 1200);
    ...
    function openCreateModal(){
        ...
        _createLock = true;
        setTimeout(()=>{ _createLock = false; }, 500);
        const body = [
            '<div class="row g-3">',
    ...
    if (window.jQuery) {
        ...
        window.__cs.createHandler = function(e){ if (e.target && e.target.closest('#cs-crear')) openCreateModal(); };
    ...
    $(document).on('click','#pt-save', function(){
        ...
        $.ajax({ type:'POST', url:'/shoppi/obtener', dataType:'json', contentType:'application/json', data: JSON.stringify(payload) })
          .done(function(resp){ app.modal.hide(); if (dt) dt.ajax.reload(null,false); loadSummary(); app.guardando(false); })
          .fail(function(){ alert('Error creando solicitud'); });
    });
    ...
    $(document).on('click','#pt-update', function(){
        ...
        $.ajax({ type:'PUT', url:'/shoppi/peticiones/'+id, dataType:'json', contentType:'application/json', data: JSON.stringify(payload) })
          .done(function(resp){ app.modal.hide(); if (dt) dt.ajax.reload(null,false); loadSummary(); })
          .fail(function(){ alert('Error actualizando solicitud'); });
    });
    ...
    function bindModalRouting(){
        ...
        if (hasGoogle && elMap) { map = new google.maps.Map(elMap, {center:{lat:7.1193,lng:-73.1227}, zoom:7}); ds = new google.maps.DirectionsService(); dr = new google.maps.DirectionsRenderer(); dr.setMap(map); }
        const calc = function(){
            ...
            if (ds) {
                ds.route({ origin:o, destination:d, travelMode: google.maps.TravelMode.DRIVING }, function(res, status){
                    if (status==='OK') {
                        ...
                        const fecha = document.getElementById('pt-fecha')?.value||''; const hora = document.getElementById('pt-hora')?.value||'00:00';
                        let dep = new Date(); if (fecha) { const [dd,mm,yy]=fecha.split('/'); const [hh,mn]=hora.split(':'); dep = new Date(+yy, +mm-1, +dd, +hh, +mn); }
                        const arr = new Date(dep.getTime()+leg.duration.value*1000);
                        $('#pt-out').text(dep.toLocaleString()); $('#pt-arr').text(arr.toLocaleString());
                        let peajes=0; leg.steps.forEach(s=>{ if ((s.instructions||'').toLowerCase().includes('toll')) peajes++; });
                ...
                fetch(url, { method:'POST', headers:{'Content-Type':'application/json','Accept':'application/json'}, body: JSON.stringify(payload) })
                  .then(r=>r.ok?r.json():Promise.reject(r)).then(resp=>{ if (resp && resp.estado===true && resp.salida){ const s=resp.salida; $('#pt-dist').text(s.distancia||'—'); $('#pt-dur').text(s.duracion||'—'); $('#pt-km').text(s.km||'—'); } });
            }
        };
        ['pt-origen','pt-destino','pt-fecha','pt-hora','pt-tipo'].forEach(id=>{ const el=document.getElementById(id); if (el){ el.addEventListener('input', calc); el.addEventListener('change', calc); } });
        setTimeout(calc, 10);
    }
    ...
    $(document).on('click','#pt-confirm-del', function(){
        ...
        $.ajax({ type:'DELETE', url:'/shoppi/peticiones/'+id, dataType:'json' })
          .done(function(){ app.modal.hide(); if (dt) dt.ajax.reload(null,false); loadSummary(); })
          .fail(function(){ alert('Error eliminando solicitud'); });
    });
}
